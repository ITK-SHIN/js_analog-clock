const AnalogClock = ($container) => {
    ...
    function setClock(hand) {
        ...
        $hourHand.style.setProperty(
            "--deg",
            hour * (360 / 12) + minute * (360 / 12 / 60) + second * (360 / 12 / 60 / 60)
        );
        $minuteHand.style.setProperty("--deg", minute * (360 / 60) + second * (360 / 60 / 60));
        $secondHand.style.setProperty("--deg", second * (360 / 60));
    }
    // 1초 있다가 실행. 세 번째 인수는 1초마다 호출될 때 주겠다.
    setInterval(setClock, 1000, [...$container.querySelectorAll(".hand")], console.log(123)); //스프레드 문법: 집합을 펼쳐서 개별 값 목록으로 만든다.
};
